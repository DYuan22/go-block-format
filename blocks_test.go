...
func TestBlocksWithSignatureProofKey(t *testing.T) {
	data := []byte("some data")
	...
	// Check the RawData method
	if !bytes.Equal(block.RawData(), data) {
		t.Errorf("RawData does not match the original data")
	}

	// Check the Signature method
	if !bytes.Equal(block.Signature(), signature) {
		t.Errorf("Signature does not match the original signature")
	}

	// Check the Proof method
	if !bytes.Equal(block.Proof(), proof) {
		t.Errorf("Proof does not match the original proof")
	}

	// Check the Key method
	if !bytes.Equal(block.Key(), key) {
		t.Errorf("Key does not match the original key")
	}
}
